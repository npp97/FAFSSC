...
root_rst<-as.data.frame(matrix(NA,ncol=7,nrow=nrow(crv_p)))
names(root_rst)<-c("ZONE","MODEL","VARIABLE","ROOT","Yhat","Yhat_min","Yhat_max")
...
#		AA<-coef(gcFitModel(x,grofit.y)$nls)[1]
		...
		grofit.y1=A*x1^B*exp(c0*x1^d)+E
		root_rst[i,6:7]<-range(grofit.y1)
	...
	if(crv_p$model[i] == "Srichards"){
		...
		grofit.y1<-A0*(1+(D0-1)*exp(-K0*(x1-xc1)))^(1/(1-D0))
		root_rst[i,6:7]<-range(grofit.y1)			
		rm(A0,xc1,D0,K0);
	}
}
...
roots.w<-reshape(root_rst,direction='wide',idvar=c("ZONE"),timevar="VARIABLE")
